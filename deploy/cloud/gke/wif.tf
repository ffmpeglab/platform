...
# change, so a copy in a repository secret goes stale. The runner exchanges its
# GitHub OIDC token for a short-lived Google one instead, so credentials are
# current by construction and there is no long-lived key to leak.
