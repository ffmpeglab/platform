...
# Nodes need to pull images and ship logs, nothing else. Credentials reach the
# pods from a Secret, so a node never holds anything of its own.
...
resource "google_container_cluster" "this" {
  ...
  # The L7 backend exists to serve Ingress, and the API is reached through a
  # Service or port-forward.
  addons_config {
    http_load_balancing {
      disabled = true
    }
